/**
 * Mobile discovery panel — full-screen overlay for browsing search results.
 *
 * Reuses the desktop DiscoveryItem component for result rendering.
 * Supports:
 * - Viewing results from a word search (auto-populated)
 * - Viewing cached results from a specific node (via Layers icon)
 * - Manual search with mode selector
 * - Adding results to the workspace (connects to the source node)
 */
...
import DiscoveryItem from '../../features/discovery/DiscoveryItem'
...
export default function MobileDiscoveryPanel() {
  ...
  // Resolve source verse key for display
  const nodes = useStore(s => s.nodes)
  const sourceVerseKey = activeNodeId
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ? nodes.find((n: any) => n.id === activeNodeId && n.type === 'verse')?.data?.verse?.verse_key ?? null
    : null
  ...
  // Close mode dropdown on outside tap
  useEffect(() => {
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    if (trimmed) searchDiscovery(trimmed)
  }, [inputValue, searchDiscovery])

  const title = sourceVerseKey
    ? `${t('title', 'Discovery')} · ${sourceVerseKey}`
    : t('title', 'Discovery')

  return (
    <MobileFullScreenPanel title={title}>
      {/* Search header */}
      <div className="px-3 py-3 border-b border-slate-100 bg-slate-50 space-y-2 shrink-0">
        ...
        <div className="flex gap-2 items-center">
          <form onSubmit={handleSubmit} className="relative flex-1">
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-full border border-emerald-300 bg-white text-slate-700 text-sm font-arabic font-medium focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
              dir="rtl"
            />
            {discoveryLoading ? (
              <Loader2 size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-500 animate-spin" />
            ) : (
              <button type="submit" className="absolute left-2 top-1/2 -translate-y-1/2 p-1 rounded-full">
          ...
          <div ref={modeRef} className="relative">
            <button
              ...
              className="h-9 px-2.5 rounded-full shadow-sm border text-[11px] font-bold flex items-center gap-1.5 bg-white text-slate-700 border-slate-200 whitespace-nowrap"
            >
                ...
                {MODE_OPTIONS.map(({ key, label, dot }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold text-left ${
        ...
        {discoverySearchMode && results.length > 0 && !discoveryLoading && (
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500 flex-wrap">
            ...
            {discoverySearchMode} · {results.length} {t('results_found_short', 'results')}
            {currentSearchTerm && (
              <span className="font-arabic font-semibold text-slate-700" dir="rtl">"{currentSearchTerm}"</span>
            )}
            {sourceVerseKey && (
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {discoveryLoading ? (
          ...
          <div className="text-center py-16 text-slate-400">
            <p className="text-sm">{t('no_results', 'No results')}</p>
          </div>
        )}
      </div>
